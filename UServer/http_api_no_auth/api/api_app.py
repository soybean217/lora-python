...
from http_api_no_auth.api.decorators import app_belong_to_user
...
from http_api_no_auth.http_auth import auth
...
def app_list(user):
    if user is not None:
        if request.method == 'GET':
            ...
            logger.info('TIMESTAMP \'apps\' obj_to_dict_over:%s' % time.time())
            respond_data = json.dumps(app_dict)
            logger.info('TIMESTAMP \'apps\' app_dict_over:%s' % time.time())
            ...
            if add_app.validate():
                try:
                    print(add_app['app_eui'].data)
                    print(type(add_app['appkey'].data))
                    logger.info('TIMESTAMP \'apps\' QueryBegin:%s' % time.time())
