...
class OTAAMsg:
    fields = ('dev_eui', 'app_eui', 'request_msg', 'dl_settings', 'rx_delay', 'cf_list')
    ...
    def publish(self):
        ...
        db0.publish(Channel1.join_req_alarm + hexlify(self.app_eui).decode(), data)
        Logger.info(action=Action.otaa, msg='Publish Join Request %s, %s' % (Channel1.join_req_alarm, data))
...
class OTAAMsgDn:
    ...
    def publish(self):
        data = json.dumps(self.__obj_to_dict())
        # db1.publish(Channel1.join_req_alarm, data)
        db0.publish(Channel1.join_accept_alarm + hexlify(self.app_eui).decode(), data)
        Logger.info(action=Action.otaa, msg='Publish Join Accept alram %s, %s' % (Channel1.join_req_alarm, data))


class JoinTransParams(TransParams):
    def save(self):
        pipe = db1.pipeline()
        key = ConstDB0.trans_params + hexlify(self.dev_eui).decode() + ':' + hexlify(self.gateway_mac_addr).decode()
        pipe.delete(key)
    ...
    class objects:
        ...
        def get(dev_eui, gateway_mac_addr, name=None):
            ...
            if name is None:
                ...
                trans_param = db1.hget(ConstDB0.trans_params + hexlify(dev_eui).decode() + ':' + hexlify(gateway_mac_addr).decode(), name)
                return trans_param.decode() if trans_param else trans_param


class JoinDevGateway(DevGateway):
    # def add(self):
    #     return db1.zadd(ConstDB0.dev_gateways + hexlify(self.dev_eui).decode(), self.cal_score(), self.gateway_mac_addr)
    ...
    def add(self):
        ...
        if not db1.execute_command('zadd', key, 'NX', score, self.gateway_mac_addr):
            Logger.debug(action='JoinDevGateway', type=IDType.device, id=self.dev_eui, msg='zadd %s, %s already exists' % (key, hexlify(self.gateway_mac_addr).decode()))
    ...
    def set_best(cls, dev_eui, gateway_mac_addr):
        db1.zadd(ConstDB0.dev_gateways + hexlify(dev_eui).decode(), 100, gateway_mac_addr)

    @classmethod
    def get_best_mac_addr(cls, dev_eui):
        key_list = db1.zrevrange(ConstDB0.dev_gateways + hexlify(dev_eui).decode(), 0, 0)
        if len(key_list) > 0:
            return key_list[0]
        else:
            return None
    ...
    def list_mac_addr_by_score(cls, dev_eui):
        key_list = db1.zrevrange(ConstDB0.dev_gateways + hexlify(dev_eui).decode(), 0, -1)
        return key_list
...
class JoiningDev:
    ...
    class objects:
        ...
        def get(dev_eui):
            ...
            if info:
                join_dev = JoiningDev(dev_eui=dev_eui, app_eui=info[b'app_eui'])
                return join_dev